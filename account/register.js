...
export class register extends React.Component {
	...
	register_click = () => {
		if (this.username_check() && this.password_check() && this.password_r_check() && this.email_check() && this.secret_q_check() && this.secret_a_check)
			WS.send(constructMessage( "register", [ this.state.username, SHA256(this.state.password), this.state.email, this.state.secret_q, this.state.secret_a.length > 0 ? SHA256(this.state.secret_a) : ""]) )
	}
	username_check = () => {
